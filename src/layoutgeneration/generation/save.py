...
def save_final_regions(root: RegionNode, output_path, save_img: bool = False):
    ...
    final_regions = list(
        map(lambda val: {"x": int(val["region"].x),
                         "y": int(val["region"].y),
                         "width": int(val["region"].width),
                         "height": int(val["region"].height),
                         "orient": val["region"].orientation.value,
                         "dir_path": val["region"].region_type.dir_path,
                         "path": val["path"],
...
def find_bfs(root: RegionNode, validate: Callable) -> List[Dict]:
    ...
    while not q.empty():
        ...
        if validate(node):
            regions.append(
                {"region": node.region, "path": os.path.join(path, f"{node.region.region_type.name}_{node.name}")})

...
def check_location(x, y, maxw, maxh, block_size):
    """
    Check if a given location is within the specified boundaries.

    Parameters:
        x (int): The x-coordinate of the location.
        y (int): The y-coordinate of the location.
        maxw (int): The maximum width of the boundary.
        maxh (int): The maximum height of the boundary.
        block_size (int): The size of the block.

    Returns:
        bool: True if the location is within the boundaries, False otherwise.
    """
...
class DrunkardsWalk(BaseStrat):
    ...
    def generate(self, region: Region) -> List[Region]:
        ...
        arr = np.empty((max_width, max_height, 4), dtype=np.int16)
